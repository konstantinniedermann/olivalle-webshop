...
- **Rabattcode (optional)** — wird gegen Gültigkeit, Einlöse-Limit und Mindestbestellwert geprüft, bevor er den Total reduziert. Das globale Einlöse-Limit (`max_einloesungen`) wird beim Buchen zusätzlich atomar durchgesetzt: ein bedingtes `UPDATE ... WHERE aktuelle_einloesungen < max_einloesungen` verhindert, dass parallele Bestellungen den Code über das Limit hinaus einlösen (Race-Absicherung). Der Per-E-Mail-Doppeleinsatz ist separat über `UNIQUE(rabattcode_id, email)` abgefangen.
- **Bestellung speichern (atomar)** — Kunde, Bestellung, Positionen, eine allfällige Rabattcode-Einlösung und (bei Stripe) die Checkout-Session-ID werden in **einer** Transaktion geschrieben. Schlägt ein Schritt fehl — z.B. die Stripe-Session-Erstellung —, wird alles zurückgerollt; es bleiben keine Waisen (Kunde ohne Bestellung o.ä.) zurück. Die Commit-Verantwortung liegt in der Router-Schicht, die Repositories committen nicht selbst.
...
- **E-Mail-Fehlertoleranz** — ein Brevo-Ausfall (oder Template-Fehler) kippt die Bestellung nicht: sie bleibt erfolgreich gespeichert, der fehlgeschlagene Versand wird als `email_fehler` im Audit-Log (`admin_log`) protokolliert — seit v1.4.12 samt Grund (Statuscode und Meldung des Anbieters), weil das Container-Log auf fly.io nach kurzer Zeit verfällt. Bei Verdacht auf fehlende Mails dort nachschauen und manuell nachsenden (Issue #162).
